...
# read in the litter wet-dry mass data
WetDry <- read_csv(
  file.path(DERT_Archives, "OngavaDungDecay","massloss_analyses",
            "data", "WetDry.csv"), 
  show_col_types = FALSE)
...
```{r}
# plot InitialDryMass versus FinalDryMass to visualize transport loss
...
```{r}
# calculate transport loss correction factor for each litter type
...
# plot of ash correction by litterbag number for each litter type
p5 <- AshMass |> 
  ggplot(aes(x = LitterbagID, y = ProportionAsh)) +
    geom_point() +
    labs(x = "Litterbag Number", y = "Wet-Dry Correction Factor") +
    theme_minimal() +
    ggtitle("Wet-Dry Correction for Litterbags")
p5
